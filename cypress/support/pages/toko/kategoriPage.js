...
const {locator_kategori, asserts} = require('../../locators/locatorsGlobal')

class Kategori {

    async clickKategoriButton() {
        cy.xpath(locator_kategori.btn_page_kategori).click()
        cy.xpath(asserts.assert_kategori).should('be.visible')
        cy.url().should('contain', urls.categoriesUrl)
    }

    async clickTambahButton(){
        cy.xpath(locator_kategori.btn_tambah).click()
    ...
    async performCreateKategori(){
        cy.fixture("kategori").then((data) => {
            data.forEach((kategori_data) => {
                cy.xpath(locator_kategori.input_nama).type(kategori_data.nama);
                cy.xpath(locator_kategori.input_deskripsi).type(kategori_data.deskripsi);
                cy.xpath(locator_kategori.btn_simpan).click();
    ...
    async performDeleteAllKategori(){
        cy.fixture("kategori").then((data) => {
            data.forEach((kategori_data) => {
                ...
                cy.xpath(locator_kategori.btn_confirm_delete).click();
